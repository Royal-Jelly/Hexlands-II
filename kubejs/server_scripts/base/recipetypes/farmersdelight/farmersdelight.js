...
function rawhideRecipe(fish, filletCount) {
    return cuttingRecipe(
        Ingredient.of(fish),
        Ingredient.of('#forge:tools/knives'),
        [Item.of('kubejs:hide_cleaned', filletCount)]
    )
}

onEvent('recipes', (event) => {
    var cuttingRemovals = [ 'minecraft:leather', 'farmersdelight:tree_bark' ];

    cuttingRemovals.forEach((removal) => {
        event.remove({ type: 'farmersdelight:cutting', output: removal });
    });
    ...
    const recipes = [
        ...
        rawhideRecipe('#forge:rawhide_small', 1),
        rawhideRecipe('#forge:rawhide_medium', 2),
        rawhideRecipe('#forge:rawhide_large', 3),
        rawhideRecipe('quark:ravager_hide', 4)
    ];
    ...
    buildWoodVariants.forEach((variant) => {
        ...
        woodRecipes.forEach((recipe) => {
            ...
            let result = [Item.of(recipe.output), Item.of(variant.strippedBark)];
